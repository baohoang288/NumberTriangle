...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while (line != null) {
            ...
            // Create NumberTriangle nodes for each number in this row
            ...
            // If this is the first row, set it as the top
            if (rowIndex == 0) {
                top = currentRow.get(0);
            } else {
                // Connect this row to the previous row
                List<NumberTriangle> prevRow = rows.get(rowIndex - 1);

                for (int i = 0; i < prevRow.size(); i++) {
                    NumberTriangle parent = prevRow.get(i);

                    // Each parent connects to two children in the current row
                    // Parent at position i connects to children at positions i and i+1
